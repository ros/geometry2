...
def main():
    ...
    print b.transform(tf2_py.Stamped(v, rospy.Time(2), 'a'), 'b')

    f = PyKDL.Frame(PyKDL.Rotation.RPY(1,2,3), PyKDL.Vector(1,2,3))
    print b.transform(tf2_py.Stamped(f, rospy.Time(2), 'a'), 'b')

    t = PyKDL.Twist(PyKDL.Vector(1,2,3), PyKDL.Vector(4,5,6))
    print b.transform(tf2_py.Stamped(t, rospy.Time(2), 'a'), 'b')

    w = PyKDL.Wrench(PyKDL.Vector(1,2,3), PyKDL.Vector(4,5,6))
    print b.transform(tf2_py.Stamped(w, rospy.Time(2), 'a'), 'b')

def convert():
    v = PyKDL.Vector(1,2,3)
    vs = tf2_py.Stamped(v, rospy.Time(2), 'a')
    vs2 = tf2_py.convert(vs, PyKDL.Vector)
    vs2[1] = 100
    print vs2
    print v
